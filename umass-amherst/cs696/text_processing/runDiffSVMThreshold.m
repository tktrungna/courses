% using liblinear's svm to train and test result, changing svm's
% thresvalue to figure precision and recall rates
clear; close all
addpath '/Users/tktrung/Downloads/liblinear-1.96/matlab'
...
for i=1:size(FEAT.TRAIN{1},2),
    ...
    svmStruct = train(DATA.TRAIN{1},group,'-c 1');
    label = predict(group,DATA.TEST{1},svmStruct);
    predict = [predict,label];
end
%predict
%%
